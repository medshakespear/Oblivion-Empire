...
# ══════════════════════════════════════════════════════════════════════
#
#  HOW GUESS THE HERO WORKS — IMPORTANT:
#
#  The bot does NOT do image recognition. It works like a pub quiz:
#    1. Bot picks a hero NAME from its own list  (bot already knows answer)
#    2. Bot posts that hero's IMAGE               (you supply URLs below)
#    3. Players TYPE the hero name in chat
#    4. Bot checks if text typed == hero name it already knows
#    5. First correct answer wins the point
#
#  HOW TO ADD IMAGES:
#    1. Find the hero in HOK_HEROES below
#    2. Paste its image URL into the "image" field
#    3. Use cropped screenshots for harder rounds
#    4. Push to GitHub — Railway redeploys automatically
#    Heroes with empty "image" are skipped until you add one.
#
#  ACCEPTED HERO NAME FORMATS (players can type any of these):
#    - Full name:      "Li Bai"
#    - No spacing:     "libai"
#    - Alias:          "ganduo" → Gan Jiang & Mo Ye
#    - Partial (3+ chars matching start): "sun sha" → Sun Shangxiang
#    Add custom aliases in HERO_ALIASES below.
#
# ══════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════
#  FULL HONOR OF KINGS HERO ROSTER
#
#  "image" : paste your URL here.  Empty = hero is skipped in game.
#  "class" : Tank / Fighter / Assassin / Mage / Marksman / Support
#  "lane"  : Baron Lane / Jungle / Mid Lane / Dragon Lane / Roam
# ══════════════════════════════════════════════════════════════════════
...
HOK_HEROES: dict[str, dict] = {
    ...
    "Nu Wa":             {"image": "", "class": "Support",   "lane": "Roam"},
    "Zhang Fei":         {"image": "", "class": "Tank",      "lane": "Roam"},
}

# ── Hero name aliases ─────────────────────────────────────────────────
# Add aliases here so players can type shortcuts and still get credit.
# Key   = what the player types (will be normalised to lowercase)
# Value = exact hero name as it appears in HOK_HEROES above
...
# ── Hero name normaliser ──────────────────────────────────────────────

def _norm(text: str) -> str:
    """
    Lowercase, remove accents, collapse spaces, strip punctuation.
    Used only for comparison — display names are unchanged.
    """
...
def check_guess(answer: str, correct_hero: str) -> bool:
    """
    Returns True if the player's typed answer matches the correct hero.

    Checks in order:
      1. Alias table lookup
      2. Direct normalised match  (e.g. "li bai" == "Li Bai")
      3. No-space match           (e.g. "libai"  == "Li Bai")
      4. Prefix match (≥4 chars)  (e.g. "sun sha" matches "Sun Shangxiang")

    The bot already knows the correct hero — this just checks the player's text.
    """
    a   = _norm(answer)
    cor = _norm(correct_hero)

    # 1. Alias lookup
    if a in HERO_ALIASES and HERO_ALIASES[a] == correct_hero:
        return True
    # Also check all alias keys that point to this hero
    for alias_key, alias_val in HERO_ALIASES.items():
        if alias_val == correct_hero and a == _norm(alias_key):
            return True
    ...
    # 3. No-space match  (handles "libai", "zhugeLiang", etc.)
    if a.replace(" ", "") == cor.replace(" ", ""):
        return True

    # 4. Prefix match — player typed ≥4 chars that start the normalised name
    if len(a) >= 4 and cor.startswith(a):
        return True
...
def get_playable_heroes() -> dict[str, dict]:
    """Only heroes that have an image URL configured."""
    return {k: v for k, v in HOK_HEROES.items() if v.get("image", "").strip()}

# ══════════════════════════════════════════════════════════════════════
#  ACTIVE GAME STATE  (per-channel, in-memory only)
# ══════════════════════════════════════════════════════════════════════

active_guess: dict[int, dict]       = {}   # channel_id → game dict
...
async def _post_round(channel: discord.TextChannel, channel_id: int):
    ...
    if not playable:
        await channel.send(embed=discord.Embed(
            ...
            description=(
                ...
                "3. For each hero, paste an image URL into the `\"image\"` field\n"
                "4. Push to GitHub — Railway redeploys automatically\n\n"
                "**Example:**\n"
                "```\n\"Li Bai\": {\"image\": \"https://i.imgur.com/abc123.png\", "
                "\"class\": \"Assassin\", \"lane\": \"Jungle\"}\n```\n\n"
    ...
    if not pool:
        await _end_guess_game(channel, channel_id); return
    ...
    e = discord.Embed(
        ...
        description=(
            ...
            "⏱ **60 seconds** — use the buttons to skip or end."
        ),
        color=EMPIRE_GOLD)
...
async def _next_round(channel: discord.TextChannel, channel_id: int):
    if not active_guess.get(channel_id): return
...
async def _end_guess_game(channel: discord.TextChannel, channel_id: int):
    ...
    if scores:
        ...
        e.add_field(name="📊 Scores", value="No points scored this game.", inline=False)
    brand(e)
...
MAFIA_ROLES: dict[str, dict] = {
    ...
    "Detective": {"emoji": "🔍", "team": "village", "desc": "Investigate one player per night — learn if they're Mafia."},
...
def _lobby_embed(game: MafiaGame) -> discord.Embed:
    e = discord.Embed(
        ...
        description=(
            ...
            "**Roles:**\n"
            "🗡️ **Mafia** — eliminate villagers at night  (1 per 3 players)\n"
            "🏡 **Villagers** — vote out Mafia by day\n"
            "💊 **Doctor** — saves one player per night  *(5+ players)*\n"
            "🔍 **Detective** — investigates one player per night  *(7+ players)*"
        ), color=EMPIRE_PURPLE)
    e.add_field(
        name=f"👥 Players  ({len(game.players)})",
        value="\n".join(f"• {m.display_name}" for m in game.players) or "—",
        inline=False)
    e.set_footer(text="⚜ Oblivion Empire | Mafia — click Join to enter!")
    return e
...
async def _start_mafia(game: MafiaGame):
    ...
    for member in game.players:
        ...
        if role == "Mafia":
            ...
            e.add_field(name="🗡️ Your Mafia", value=team_names, inline=False)
    ...
    e = discord.Embed(
        ...
        description=(
            ...
            + (f"\n⚠️ DMs failed: {', '.join(dm_failures)}" if dm_failures else "")
        ), color=EMPIRE_PURPLE)
    brand(e)
...
async def _day_phase(game: MafiaGame):
    ...
    if winner: await _end_mafia(game, winner); return
    ...
    if not game.votes:
        await game.channel.send(embed=discord.Embed(
            ...
            description="Nobody eliminated. Night falls...",
            color=EMPIRE_PURPLE))
    else:
        tally: dict[int, int] = {}
...
async def _night_phase(game: MafiaGame):
    ...
    for member in game.alive:
        ...
        action_e = discord.Embed(
            ...
            description="Use the menu to take your action.",
            color=EMPIRE_PURPLE)
    ...
    if invest_id:
        ...
        if target and det:
            ...
            try: await det.send(embed=result_e)
            except Exception: pass
    ...
    if elim_id and elim_id != prot_id:
        ...
        if victim and victim in game.alive:
            ...
            dawn_lines.append(f"💀 **{victim.display_name}** was eliminated overnight.\n"
    ...
    e = discord.Embed(title=f"🌅 Dawn — Day {game.day + 1}",
                      description="\n".join(dawn_lines), color=EMPIRE_GOLD)
    brand(e)
    await game.channel.send(embed=e)
    winner = game.check_win()
    if winner: await _end_mafia(game, winner); return
...
class GamesPanelView(View):
    ...
    async def guess_hero_btn(self, interaction: discord.Interaction, _: Button):
        ...
        if not playable:
            await interaction.response.send_message(
                embed=discord.Embed(
                    title="⚠️ No Hero Images Configured",
                    description=(
                        "**How to add images and enable this game:**\n\n"
                        "1. Open `games.py`\n"
                        "2. Find `HOK_HEROES` dictionary\n"
                        "3. Paste image URLs into the `\"image\"` field for each hero\n"
                        "4. Push to GitHub — Railway redeploys\n\n"
                        "**The bot already knows the answers** — it picks a hero, posts its image,\n"
                        "then waits for players to type the name. No image recognition needed."
        ...
        e = discord.Embed(
            ...
            description=(
                ...
                "**Type the hero's name in this channel** — first correct answer wins a point!\n\n"
        ...
        await log_action(interaction.guild, "🔎 Guess Hero",
    ...
    async def scores_btn(self, interaction: discord.Interaction, _: Button):
        ...
        for i, (uid, pts) in enumerate(sorted_s[:15], 1):
            ...
            medal = medals[i - 1] if i <= 3 else f"`{i}.`"
            lines.append(f"{medal} **{name}** — {pts} pt{'s' if pts != 1 else ''}")
        e = discord.Embed(title="🏅 Guess the Hero — All-Time Scores",
...
class GamesCog(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        """Check every chat message in active Guess the Hero channels."""
        if message.author.bot: return
        cid  = message.channel.id
        game = active_guess.get(cid)
        if not game or game.get("revealed"): return
        hero = game.get("hero")
        if not hero: return
