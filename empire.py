...
class EmpirePanelView(View):
    ...
    async def expedition_btn(self, i: discord.Interaction, _: Button):
        ...
        await i.response.send_message(embed=e,
            view=ExpeditionView(uid, scenario, i.channel_id), ephemeral=True)
    ...
    async def dungeon_btn(self, i: discord.Interaction, _: Button):
        ...
        e = discord.Embed(
            title="🏰  الزنزانة — الغرفة 1/3",
            description=(
                f"{SEP}\n"
                f"دخلت الزنزانة بـ **100** نقطة حياة.\n\n"
                f"**الغرفة الأولى:** {first_room['emoji']} {first_room['name']}\n\n"
                "اختر طريقتك في التعامل:\n"
                f"{SEP}"
            ), color=TEAL)
        brand(e)
        await i.response.send_message(embed=e, view=DungeonRoomView(i.user.id, 0), ephemeral=True)
    ...
    async def duel_btn(self, i: discord.Interaction, _: Button):
        ...
        brand(e)
        await i.response.send_message(embed=e, view=DuelTargetView(i.guild.get_member(i.user.id), i.guild), ephemeral=True)
    ...
    async def oracle_btn(self, i: discord.Interaction, _: Button):
        ...
        e = discord.Embed(
            ...
            description=(
                f"{SEP}\n"
                f"*تُنفق 100 🖤 … العرّاف يرفع عينيه…*\n\n"
                f"**❝  {prophecy}  ❞**\n\n"
                f"*النبوءة تنطبق على مغامرتك القادمة.*\n"
                f"{SEP}"
            ), color=PHANTOM)
        brand(e)
        await i.response.send_message(embed=e, ephemeral=True)
...
class EmpireCog(commands.Cog):
    ...
    async def cmd_empire(self, i: discord.Interaction):
        ...
        e = discord.Embed(
            title="⚔️  سجلات الإمبراطورية المظلمة",
            description=(
                f"{SEP}\n"
                f"*مرحباً، **{i.user.display_name}**.*{member_rank}\n\n"
                "**🌑 ارتقِ في الرتب · اجمع الشظايا · اكتشف الأسرار**\n\n"
                f"👥 **{members_count}** محارب مسجّل في الإمبراطورية\n"
                f"{SEP}\n"
                "اختر أحد الأزرار أدناه لبدء مغامرتك:"
            ), color=DARK_EMPIRE)
        e.set_thumbnail(url=logo_url() or (bot_avatar() or discord.utils.MISSING))
        e.add_field(name="⚔️ شخصيتي",       value="أنشئ شخصيتك أو راجع إحصائياتك",  inline=True)
        e.add_field(name="🎯 مغامرة",        value="مغامرة يومية بخيارات قصصية",       inline=True)
        e.add_field(name="🏰 الزنزانة",      value="زنزانة أسبوعية من 3 غرف",          inline=True)
        e.add_field(name="⚔️ مبارزة",        value="تحدّ عضواً آخر لمعركة 3 جولات",   inline=True)
        e.add_field(name="🏪 المتجر",         value="أنفق شظاياك على الأدوار والمعدات", inline=True)
        e.add_field(name="🔮 العرّاف",        value="100 🖤 للحصول على نبوءة",          inline=True)
        e.set_footer(text="⚜  الإمبراطورية المظلمة  ·  سجلات الشرف",
                     icon_url=logo_url() or discord.utils.MISSING)
